...
contract Staker is ReentrancyGuard {
    ...
    function requestStakeTokens() external nonReentrant{
        require(block.timestamp >= lastRequestTime[msg.sender]+ 1 days,"Too many requests, try after 24 hours");
        uint256 contractBalance = s_stakingToken.balanceOf(address(this));
        require(contractBalance >= 2000000000000000000,"We are out of BLK tokens, please try again later!");
        bool success = s_stakingToken.transfer(msg.sender, 2000000000000000000);
